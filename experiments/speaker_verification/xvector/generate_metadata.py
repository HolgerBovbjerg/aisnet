...
def generate_LibriSpeech_metadata(root, save_path):
    filepaths = []
    for file in Path(root).rglob("*.flac"):
        filepaths.append(file)
    ...
    for file in tqdm(filepaths, total=len(filepaths)):
        ...
        num_channels = info.num_channels
        duration_seconds = num_frames / sample_rate
        relative_file_path = Path(file).relative_to(root)
        data.append(
            [relative_file_path, sample_rate, num_frames, bits_per_sample, encoding, num_channels, duration_seconds,
             speaker_id, chapter_id, utterance_id])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--features", action="store_true", required=False, help="Whether the data are precomputed features.")
