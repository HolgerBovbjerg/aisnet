...
from xvector.data_loader import build_xvector_datapipe, build_voxceleb1_test_datapipe, get_data_loader, pad_collate, \
    voxceleb1_test_collate
from xvector.loss import AdditiveAngularMarginSoftmax
...
def train_one_batch(model, features, lengths, labels, criterion, spec_augmentor, optimizer, scheduler, device):
    if spec_augmentor:
...
def training_pipeline(config):
    ...
    train_datapipe, train_label_to_speaker_id = build_xvector_datapipe(data_sets=config["data"]["train_data"],
                                                                       ...
                                                                       segment_max_size=config["data"][
                                                                           "segment_max_size"],
                                                                       max_token_count=config["data"][
                                                                           "max_token_count"],
                                                                       min_length=config["data"]["min_length"])
    train_loader = get_data_loader(dataset=train_datapipe,
                                   batch_size=config["hparams"]["batch_size"],
    ...
    # model
    if "n_classes" not in config["hparams"]["model"]:
        config["hparams"]["model"]["n_classes"] = len(train_label_to_speaker_id.keys())
    model = get_model(config["hparams"]["model"]["encoder"])
    print(f"Created model with {count_parameters(model)} parameters.")

    if "checkpoint" in config["hparams"]["model"]:
        model = load_pretrained_lstm(model,
                                     checkpoint_path=config["hparams"]["model"]["checkpoint"]["checkpoint_path"],
                                     map_location="cpu")
        if config["hparams"]["model"]["checkpoint"]["freeze"]:
            freeze_model_parameters(model.lstm)
            print(f"{count_parameters(model.lstm)} parameters frozen. "
                  f"{count_parameters(model, trainable=True)} trainable parameters.")

    model = model.to(device)

    # loss
    if config["hparams"]["loss"]["type"] == "AAMSoftmax":
        criterion = AdditiveAngularMarginSoftmax(n_out=config["hparams"]["model"]["encoder"]["settings"]["hidden_dim"],
                                                 n_classes=config["hparams"]["model"]["n_classes"]).to(device)
    else:
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            train_batch_loss = train_one_batch(model=model,
                                               features=features,
                                               lengths=lengths,
                                               labels=labels,
                                               criterion=criterion,
...
if __name__ == "__main__":
    parser = ArgumentParser("Training and evaluation script for xvector nnet.")
