...
def estimate_doa(tdoa: torch.Tensor, dist_mics: torch.Tensor, c_sound: float = 343.):
    ...
    above = dist_wave >= dist_mics
    below = dist_wave <= -dist_mics
    dist_mics = dist_mics.expand_as(above)
...
class GCCDoA(nn.Module):
    ...
    def forward(self, x):
        tdoa = self.gcc(x, output_delay=True, output_tdoa=True)
...
if __name__ == "__main__":
    ...
    gcc_phat_doa = GCCDoA(n_fft=512, window_length=400, hop_length=160, sample_rate=sample_rate,
