...
@pytest.mark.parametrize("output_type", ["raw", "power", "real_imag", "log_power", "power_phase", "log_power_phase"])
def test_stft(output_type):
    ...
    if output_type == "log_power":
        ...
        assert stft_output.shape[-1] == (
                    2 * (stft.n_fft // 2 + 1)), "Real Imag output should have 2 * (n_fft // 2 + 1) features."
        ...
        assert stft_output.shape[-1] == (2 * (stft.n_fft // 2 + 1)), "LogPower-phase output should have 2 * (n_fft // 2 + 1) features."
        # Ensure log power is valid
        assert torch.all(~stft_output.isnan()), "Log-power values should not be NaN."
